...
Source = AsyncIterator[Sample[SupportsFloatT]]
...
Sink = Callable[[Sample[SupportsFloatT]], Coroutine[None, None, None]]
...
Args:
    sample: A sample to be sent out.
"""
...
Args:
    input_samples: The sequence of pre-existing samples.
    resampler_config: The configuration of the resampling calling this function.
...
def average(
    samples: Sequence[Sample[SupportsFloatT]],
    resampler_config: ResamplerConfig[SupportsFloatT],
    ...
    values = list(float(sample.value) for sample in samples if sample.value is not None)
    return sum(values) / len(values)


@dataclass(frozen=True)
class ResamplerConfig(Generic[SupportsFloatT]):
    ...
    """The function to construct new values for the resampled timeseries."""
    ...
    resampling_function: ResamplingFunction[SupportsFloatT] = average
...
class SourceStoppedError(RuntimeError, Generic[SupportsFloatT]):
    """A timeseries stopped producing samples."""

    def __init__(self, source: Source[SupportsFloatT]) -> None:
        """Create an instance.
        ...
        self.source: Source[SupportsFloatT] = source
...
class ResamplingError(RuntimeError, Generic[SupportsFloatT]):
    ...
    def __init__(
        self,
        exceptions: dict[Source[SupportsFloatT], Exception | asyncio.CancelledError],
    ) -> None:
        """Create an instance.
        ...
        self.exceptions: dict[
            Source[SupportsFloatT], Exception | asyncio.CancelledError
        ] = exceptions
...
class Resampler(Generic[SupportsFloatT]):
    ...
    def __init__(self, config: ResamplerConfig[SupportsFloatT]) -> None:
        """Initialize an instance.

        Args:
            config: The configuration for the resampler.
        """
        self._config: ResamplerConfig[SupportsFloatT] = config
        """The configuration for this resampler."""

        self._resamplers: dict[
            Source[SupportsFloatT], _StreamingHelper[SupportsFloatT]
        ] = {}
    ...
    @property
    def config(self) -> ResamplerConfig[SupportsFloatT]:
    ...
    def get_source_properties(self, source: Source[SupportsFloatT]) -> SourceProperties:
    ...
    def add_timeseries(
        self, name: str, source: Source[SupportsFloatT], sink: Sink[SupportsFloatT]
    ) -> bool:
    ...
    def remove_timeseries(self, source: Source[SupportsFloatT]) -> bool:
...
class _ResamplingHelper(Generic[SupportsFloatT]):
    ...
    * max_data_age_in_periods` are considered *relevant* and are passed
    ...
    def __init__(self, name: str, config: ResamplerConfig[SupportsFloatT]) -> None:
        """Initialize an instance.
        ...
        self._name: str = name
        self._config: ResamplerConfig[SupportsFloatT] = config
        self._buffer: deque[Sample[SupportsFloatT]] = deque(
    ...
    def add_sample(self, sample: Sample[SupportsFloatT]) -> None:
    ...
    def resample(self, timestamp: datetime) -> Sample[SupportsFloatT]:
        ...
        if value is None or math.isnan(value):
            return Sample(timestamp, None)
        return Sample(timestamp, self._config.value_constructor(value))


class _StreamingHelper(Generic[SupportsFloatT]):
    """Resample data coming from a source, sending the results to a sink."""

    def __init__(
        self,
        helper: _ResamplingHelper[SupportsFloatT],
        source: Source[SupportsFloatT],
        sink: Sink[SupportsFloatT],
    ) -> None:
        """Initialize an instance.
        ...
        Args:
            ...
            sink: The sink to use to send the resampled data.
        """
        self._helper: _ResamplingHelper[SupportsFloatT] = helper
        self._source: Source[SupportsFloatT] = source
        self._sink: Sink[SupportsFloatT] = sink
