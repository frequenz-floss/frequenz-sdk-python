...
class BatteryStatusTracker:
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self._battery: _ComponentStreamStatus = _ComponentStreamStatus(
            battery_id, data_recv_timer=Timer.periodic(dt.timedelta(max_data_age_sec))
        )
        self._inverter: _ComponentStreamStatus = _ComponentStreamStatus(
            inverter_id, data_recv_timer=Timer.periodic(dt.timedelta(max_data_age_sec))
        )
    ...
    def _handle_status_battery(
        self, bat_data: BatteryData
    ...
    def _handle_status_inverter(
        self, inv_data: InverterData
    ) -> None:  # Any is probably not the best
    ...
    def _handle_status_battery_timer(
    ...
    def _handle_status_inverter_timer(
        self, _timedelta: timedelta
    ) -> None:  # Any is probably not the best
            ...
            try:
                async for selected in select(
                    ...
                    if selected_from(selected, battery):
                        ...
                        self._handle_status_inverter_timer(selected.value)
