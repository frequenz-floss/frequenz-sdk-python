...
async def run_test_battery_status_channel(  # pylint: disable=too-many-arguments
    ...
    await battery_status_sender.send(
        ComponentStatus(working=working, uncertain={batteries_in_pool[1]})
    )
    ...
    await battery_status_sender.send(ComponentStatus(working=working, uncertain=set()))
    msg = await asyncio.wait_for(
        battery_pool_metric_receiver.receive(), timeout=waiting_time_sec
    )
    ...
    await battery_status_sender.send(
        ComponentStatus(working=working, uncertain={batteries_in_pool[0]})
    )
    msg = await asyncio.wait_for(
        battery_pool_metric_receiver.receive(), timeout=waiting_time_sec
    )
    ...
    await battery_status_sender.send(
        ComponentStatus(working=set(all_batteries), uncertain=set())
    )
    msg = await asyncio.wait_for(
        battery_pool_metric_receiver.receive(), timeout=waiting_time_sec
    )
...
async def run_capacity_test(setup_args: SetupArgs) -> None:
    ...
    await battery_status_sender.send(
        ComponentStatus(working=all_batteries, uncertain=set())
    )
